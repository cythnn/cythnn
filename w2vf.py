...
def doTrain():
    return model(alpha = 0.025, vectorsize=100,
                 ...
                 build=[build_vocab, build_hierarchical_softmax, createW2V ],
                 pipeline=[readWordIds, processhs],
                 pipelinec=[addTrainW2V],
                 mintf=1, cores=2, windowsize=1, iterations=1, debugtarget=None)

def doTest(byterange=None):
    return model(alpha=0.025, vectorsize=100,
                 input=wordStreams("data/text8", byterange=byterange, parts=2),
                 build=[build_vocab, createW2V, build_hierarchical_softmax],
                 pipeline=[readWordIds, processhs],
                 pipelinec=[addTrainW2V],
...
if __name__ == "__main__":
    ...
    # model = doTest( byterange=range(1000000) ) # specify byterange to truncate the input
    # model = doTrain(), #tiny example to experiment on
